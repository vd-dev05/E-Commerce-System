...
import { createOrderPaymentSepay, editPaymentOrder } from "@/store/Shop/users/userThunk";
...
const SepayQr = () => {
    ...
    useEffect(() => {
        if (time > 0 && code) {
            ...
            dispatch(createOrderPaymentSepay(code)).then(data => {
                console.log(data);
                if (data.payload.status === 200 && data.payload.message === "Chuyển khoản thành công") {
                    toast({
                    ...
                    //     paymentSuccess: true,
                    //     address: data.dataOrder.address ,
                    //     totalAmount: payloadTransaction.amount + 10000
                    // }
                    // dispatch(editPaymentOrder({ id:payloadTransaction, payload }))
